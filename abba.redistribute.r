...
abba.redistribute = function(abbabin,pos,
...
...) {
  ...
  if (pos == length(abbabin$bin)) stop("pos cannot be the last position!!!")
   
  # attach the abbabin for easy working  
  a = abbabin

  # locate the cut points in the original bin first
  # origp1_1 is the first bin
  # origp2_2 is the last bin
  # all the bins between origp1_1 and origp2_2 combines together
  # to give the current bin at position p1
  # similarly for origp2_1 and origp2_2
  
  # p1 is position 1 
  # p2 is position 2 
  # they denote position in the abbabin
  # in which the bins to redistribute are situated
  p1 = pos
  p2 = pos + 1
  ...
  # best_cut is the best cutting point in terms of least info loss
  ...
  for (i in  origp1_1:(origp2_2-1)) {
    ...
    if (length(focus) == 0)
    {     
      tot_info_loss1 = infoloss_fn(new_b1,new_g1,new_b2,new_g2,...)   

      # i want to find the point of greatest differentiation so the bins should look as different as possible
  ...
  if (best_cut == origp1_2) return(NULL)
  else {
