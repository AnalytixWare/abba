...
abba.turning_pt_pearson_min_pop_bad_focus = function(abbabin, 
    ...
    a = abba.pearson_min_pop_bad_focus(abbabin, signif = signif, 
        min_pop = min_pop, min_bad = min_bad, ...)
...
abba.min_bad_focus = function(abbabin, min_bad = sum(abbabin$b)/20, 
    ...
    if (a[length(a)] == length(abbabin$b)) 
        a[length(a)] = length(abbabin$b) - 1
...
abba.pearson_focus = function(abbabin, signif = 0.5, ...) {
    ...
    # if statistics is small then the two bins are likely to be
    # the same
...
abba.min_woe_diff_focus = function(abbabin, min_woe_diff = 0, 
    ...
    # return the set of indices where the weight of evidence is
...
abba.trendup_pearson_min_pop_bad_min_woe_diff_max_num_bins_focus = function(...) {
    ...
    if (length(r) == 0) {
        c = abba.max_num_bins_focus(...)
        # print (paste('length0',length(c))) print(c)
        return(c)
    } else {
