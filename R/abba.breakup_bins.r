...
abba.breakup_bins = function(
  abbabin,
  pos,
  focus_fn = abba.trendup_pearson_min_pop_bad_focus,
  verbose=F,...) {

  # use a shorter name for easier working
  a = abbabin
  
  # find the pos in the original bins that merged to give this bin
  ...
  for (i in random_index) {
    ...
    if (length(focus)==0) {
      if (verbose || T) {
       print(paste("break up from (",
              new_b1+new_b2,new_g1+new_g2,") -> (",new_b1,new_g1,") + (",new_b2,new_g2,")"))
      }
      ...
      if (pos == 1) {
        ...
        a$bin = c(
          a$origbin$bin[i],
          a$origbin$bin[origpos[2]],
          a$bin[(pos+1):length(a$bin)])
      }
      else {
        a$b = c(a$b[1:(pos-1)],new_b1,new_b2,a$b[(pos+1):length(a$b)])
        a$g = c(a$g[1:(pos-1)],new_g1,new_g2,a$g[(pos+1):length(a$g)])
        a$bin = c(a$bin[1:(pos-1)],
                  a$origbin$bin[i],
                  a$origbin$bin[origpos[2]],
                  a$bin[(pos+1):length(a$bin)])
      }
